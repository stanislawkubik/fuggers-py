...
- `curve_builder.py`: calc-side registry for finished curves and remembered raw inputs. It no longer constructs curves.
...
- `curves/`: the current rates curve skeleton. The root package now exports `CurveSpec`, `CurveType`, `ExtrapolationPolicy`, `RateSpace`, `RatesTermStructure`, `DiscountingCurve`, `YieldCurve`, and `RelativeRateCurve`. `rates/spec.py` holds the curve identity record. `rates/base.py` defines the public roots: `rate_at(tenor)` returns the rate at a tenor in the curve's `rate_space`, `max_t()` gives the last supported tenor, and `validate_rate(tenor)` enforces the domain and finite-value rules. `DiscountingCurve` adds `discount_factor_at(tenor)`, `zero_rate_at(tenor)`, and `forward_rate_between(start_tenor, end_tenor)`. `YieldCurve` is now the concrete public discounting object: it wraps one internal kernel, fixes its public `rate_space` to `RateSpace.ZERO`, exposes the public zero-rate view through `rate_at(tenor)`, and now also provides `fit(quotes, spec=..., kernel_spec=...)` as the shared public construction path for the live fitting methods. `rates/reports.py` now holds `CalibrationPoint` and `CalibrationReport`. `rates/kernels/base.py` now defines the shared internal kernel vocabulary: `CurveKernelKind`, `KernelSpec`, and `CurveKernel`. The shared kernel contract is now rate-first: kernels define the fitted rate curve and derive discount factors from it. `rates/kernels/nodes.py` now holds the node-based internal kernel family: linear-zero, log-linear-discount, piecewise-constant-zero, piecewise-flat-forward, cubic-spline-zero, and monotone-convex kernels. `rates/kernels/parametric.py` now holds the parametric internal kernel family: `NelsonSiegelKernel` and `SvenssonKernel`, each with an explicit finite `max_t` even though the underlying formulas can evaluate beyond it. `rates/kernels/spline.py` now holds the spline internal kernel family: `ExponentialSplineKernel` and `CubicSplineKernel`. `rates/calibrators/base.py` defines `CalibrationObjective` and `CurveCalibrator`. `rates/calibrators/bootstrap.py` defines the node-bootstrap fitting path: `BootstrapSolverKind` and `BootstrapCalibrator`. That path now takes market quotes, normalizes supported quote types internally, and also accepts `CurveKernelKind.CUBIC_SPLINE`, so the newer cubic-spline kernel can be reached through the same public fitting flow. `rates/calibrators/parametric.py` defines the global-fit path: `ParametricOptimizerKind` and `ParametricCalibrator`, which now fits `NELSON_SIEGEL`, `SVENSSON`, and `EXPONENTIAL_SPLINE` from the same quote-driven entry point. For the exponential-spline case, callers provide fixed `decay_factors` in `KernelSpec.parameters`, and the calibrator fits the spline coefficients against those fixed decay factors. `rates/calibrators/_quotes.py` is the private quote-normalization helper for those live fitting paths. `rates/calibrators/bonds.py` is now only a reserved home if richer specialized diagnostics or helper logic still prove necessary after migration. `conversion.py` and `errors.py` are shared helpers. `multicurve/` currently holds identifiers such as `RateIndex` and `CurrencyPair`.
...
- The rebuilt package still does not include breakeven curves, par-yield curves, or richer concrete fit reports beyond the generic `CalibrationReport`. The public ontology, the node kernels, the parametric kernels, the spline kernels, the bootstrap calibrator, and the global-fit calibrator now exist.
