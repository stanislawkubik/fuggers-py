...
Volatility surface records now have their public home in
``fuggers_py.vol_surfaces``. This module still re-exports them because market
snapshots store those records directly.
"""
...
if TYPE_CHECKING:
    ...
    from fuggers_py.vol_surfaces import InMemoryVolatilitySource, VolPoint, VolQuoteType, VolSurfaceType, VolatilitySurface
...
def __getattr__(name: str) -> object:
    runtime_quote_exports = {
        "RawQuote",
        "SourceType",
    }
    if name in runtime_quote_exports:
        from fuggers_py._runtime import quotes as runtime_quotes

        return getattr(runtime_quotes, name)
    ...
    if name in volatility_exports:
        from fuggers_py import vol_surfaces

        return getattr(vol_surfaces, name)
