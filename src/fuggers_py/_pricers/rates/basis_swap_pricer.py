...
from fuggers_py._products.rates import BasisSwap, FloatingLegSpec
...
class BasisSwapPricer:
    ...
    def pay_leg_pv(self, swap: BasisSwap, curves: AnalyticsCurves) -> Decimal:
        """Return the discounted PV of the pay leg."""

        return self._leg_pv(swap, swap.pay_leg, swap.pay_periods(), curves, include_spread=True)

    def receive_leg_pv(self, swap: BasisSwap, curves: AnalyticsCurves) -> Decimal:
    ...
    def pv(self, swap: BasisSwap, curves: AnalyticsCurves) -> Decimal:
        """Return the total present value of the swap."""

        return self.pay_leg_pv(swap, curves) + self.receive_leg_pv(swap, curves)

    def par_spread(self, swap: BasisSwap, curves: AnalyticsCurves) -> Decimal:
    ...
    def price(self, swap: BasisSwap, curves: AnalyticsCurves) -> BasisSwapPricingResult:
