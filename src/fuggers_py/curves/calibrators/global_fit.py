...
from fuggers_py._math.errors import MathError
from fuggers_py._math.optimization import OptimizationConfig, OptimizationResult, gauss_newton, levenberg_marquardt
from fuggers_py._math.solvers.brent import brent
from fuggers_py._math.solvers.types import SolverConfig
from fuggers_py._math.utils import assert_finite_array, assert_strictly_increasing
...
def _is_expected_bond_diagnostic_error(exc: Exception) -> bool:
    if isinstance(exc, InvalidCurveInput):
        return True
    return any(base.__name__ == "BondError" for base in type(exc).__mro__)
...
def _bond_price_diagnostics(
    ...
    *,
    ...
    try:
        ...
        if not _is_expected_bond_diagnostic_error(exc):
            raise
