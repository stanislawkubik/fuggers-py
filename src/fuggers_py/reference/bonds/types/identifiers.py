...
from fuggers_py.core.calendars import (
    Calendar,
    JapanCalendar,
    SIFMACalendar,
    Target2Calendar,
...
def _normalize_calendar_id(value: str) -> str:
    """Normalize calendar identifiers while preserving backwards compatibility."""
...
@dataclass(frozen=True, slots=True)
class CalendarId:
    """A lightweight calendar identifier used by the bond layer.

    The bond layer maps the most common ids onto
    `fuggers_py.core.calendars` singletons.
    """
    ...
    def max_date(self) -> Date:
        # A small convenience wrapper mirroring the other identifier types.
