...
The inflation convention layer defines how index names, observation lags, and
interpolation methods are interpreted when resolving reference CPI series.
"""
...
from fuggers_py.market.snapshot import InflationInterpolation

from .errors import InvalidObservationLag
...
class InflationConvention:
    ...
    family:
        Normalized family label used for grouping related indices.
    currency:
        Currency of the linked instrument.
    index_source:
        Canonical index name used for lookups.
    observation_lag_months:
        Number of whole months between settlement and the observed CPI month.
    interpolation_method:
        Monthly or linear interpolation policy applied to the fixings.
    publication_style:
        Optional source-style label such as seasonally adjusted or not.
    aliases:
        Additional lookup names accepted by the fixing source.
    notes:
        Human-readable convention notes.
    """
