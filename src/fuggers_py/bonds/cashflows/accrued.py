"""Accrued interest calculations (`fuggers_py.bonds.cashflows.accrued`).
...
class AccruedInterestCalculator:
    ...
    def standard(inputs: AccruedInterestInputs, *, rules: YieldCalculationRules) -> Decimal:
        ...
        convention_name = AccruedInterestCalculator._accrued_convention_name(rules)

        if convention_name == "NONE":
            return Decimal(0)

        if convention_name in {"STANDARD", "EX_DIVIDEND", "RECORD_DATE", "CUM_DIVIDEND"}:
            return inputs.coupon_amount * AccruedInterestCalculator._coupon_fraction(inputs, rules=rules)

        if convention_name == "USING_YEAR_FRACTION":
            return inputs.coupon_amount * AccruedInterestCalculator._coupon_fraction(inputs, rules=rules)

        if convention_name == "ISMA":
