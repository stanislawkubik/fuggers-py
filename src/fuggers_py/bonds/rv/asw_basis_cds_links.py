...
def decompose_asw_basis_cds_links(
    *,
    ...
    asset_swap_value = asset_swap_spread if isinstance(asset_swap_spread, Decimal) else Decimal(str(asset_swap_spread))
    same_currency_value = (
        same_currency_basis if isinstance(same_currency_basis, Decimal) else Decimal(str(same_currency_basis))
    )
    cross_currency_value = (
        cross_currency_basis if isinstance(cross_currency_basis, Decimal) else Decimal(str(cross_currency_basis))
    )
    common_currency_floating_spread = asset_swap_value + same_currency_value + cross_currency_value
    adjusted_cds_value = (
        None
        if adjusted_cds_spread is None
        else adjusted_cds_spread if isinstance(adjusted_cds_spread, Decimal) else Decimal(str(adjusted_cds_spread))
    )
