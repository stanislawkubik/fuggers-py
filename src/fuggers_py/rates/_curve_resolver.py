...
def _clone_curves(curves: object, **updates: object) -> object:
    fields = {
...
def resolve_discount_curve(curves: object, currency: Currency) -> DiscountingCurve:
...
def resolve_inflation_projection(curves: object, *, convention) -> object:
...
def _parallel_bump_curve(curve: object | None, amount: float) -> object | None:
    if _curve_supports_discounting(curve):
        return cast("DiscountingCurve", curve).shifted(shift=amount)
    return curve


def _key_rate_bump_curve(curve: object | None, tenor_grid: tuple[Tenor, ...], key_tenor: Tenor, bump: float) -> object | None:
    if _curve_supports_discounting(curve):
        return cast("DiscountingCurve", curve).bumped(tenor_grid=tenor_grid, bumps={key_tenor: bump})
    return curve
